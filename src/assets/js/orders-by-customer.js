...
// Use States
const setCustomerId = (c) => {
    customerId = c;
    setCustomerIdAction();
}

const setCustomerIdAction = () => {
    title.innerHTML = `Orders By Customer: ${customerId}`;
    ...
    currentBreadcrumbA.href = `orders-by-customer.html?id=${customerId}`;
    currentBreadcrumbA.innerHTML = "Order By Customer";
...
const setOrdersAction = () => {
    table.innerHTML = "";
    orders.forEach(({id, consigneeName, address, phone, createAt, finishedAt, orderStatus}) => {
...
const useEffect = async () => {
    let customerId = parseInt(urlSearchParams.get("id"));
...
// Functions
const url = () => {
    let init = `http://localhost:8080/api/v1/orders/by-customer-lazy/${customerId}` + (currentStatus !== 0 ? `/by-status/${statusMap.find(({id}) => id === currentStatus).name}` : "");
    let query = [];
    if (currentPage !== 0) {
...
const getOrdersCountUrl = () => {
    return `http://localhost:8080/api/v1/orders/count/by-customer/${customerId}` + ((currentStatus !== 0) ? `/by-status/${statusMap.find(({id}) => id === currentStatus).name}`: "");
}
